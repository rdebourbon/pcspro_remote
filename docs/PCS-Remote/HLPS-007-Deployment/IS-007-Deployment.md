...
6. **Windows Firewall rule (idempotent)**: Removes any existing rule named `PcsRemote-HTTP` then creates a new inbound TCP rule for port `$Port` (`New-NetFirewallRule`). Note: unlike Task Scheduler, `New-NetFirewallRule` has no `-Force` atomic-replace option. The delete-then-recreate window is accepted here because a missing firewall rule is recoverable by re-running the script, whereas a missing Task Scheduler task would silently break auto-start at logon. If `New-NetFirewallRule` fails after `Remove-NetFirewallRule`, re-running the script recreates the rule — no data is lost.
...
- **Setting the PCS Pro password** (via `PcsPro__Password` System-scoped environment variable):
  ...
  - The deployment script sets this interactively. If you need to set it manually without re-running the full deployment script, use `Read-Host` to avoid the password appearing in shell history:
    ...
    ```
    The BSTR pointer is captured before conversion so it can be freed via `ZeroFreeBSTR` in the `finally` block, mirroring the security practice in `Deploy-PcsRemote.ps1` step 7. Restart the application or the Task Scheduler task for the change to take effect.
...
1. **Before the match** — How to confirm the application is running (look for the tray icon; if absent, log on to the garage PC — the application starts automatically within about 30 seconds of logon via the ONLOGON trigger). If the tray icon still has not appeared, contact your IT contact — they can restart the application by running `Start-ScheduledTask -TaskName PcsRemote` from an elevated PowerShell prompt on the garage PC.
...
5. **Manual mode** — Manual mode pauses all automation (PCS Pro is no longer controlled automatically) and blocks new remote commands immediately. If an automation action is already running when manual mode is activated, it will complete before any further remote commands are rejected — wait for the current status to settle before taking over PCS Pro directly. Use manual mode when you need to operate PCS Pro without interference (e.g., correcting an error in the scorer). To enable: right-click the tray icon on the garage PC and click "Switch to Manual Mode" — the web control panel will display a "Manual mode — automation paused by local operator" banner. To disable: right-click the tray icon and click "Resume Automation" — the banner disappears. Note: manual mode can only be toggled from the garage PC (tray icon), not from the web control panel.
...
8. **Restarting the application** — Right-click the tray icon → Exit; then log off and log on again (the application restarts automatically within about 30 seconds of logon via the ONLOGON Task Scheduler trigger). Note: the 30-second restart delay configured for crash recovery does not apply here; a clean exit and re-logon restarts the application immediately.
9. **After a reboot** — If the garage PC has been restarted (e.g., after a power cut or Windows Update): log on to the garage PC if auto-logon is not configured; the application will start automatically within about 30 seconds of logon (ONLOGON trigger).
...
**Models**: Claude Opus 4.6 · GPT-5.4 · **Result**: APPROVED — 7 findings accepted (4 MEDIUM, 2 LOW, 1 INFO)

| ID | Finding | Severity | Source | Disposition |
|---|---|---|---|---|
| R9-M1 | S-003 manual password snippet leaks BSTR — `SecureStringToBSTR` result discarded inline, `ZeroFreeBSTR` never called; contradicts S-002 step 7 security discipline | MEDIUM | Opus + GPT | Accepted — snippet rewritten to capture BSTR in `$bstr`, wrap conversion in `try/finally`, call `ZeroFreeBSTR($bstr)` in `finally` |
| R9-M2 | S-002 step 6 firewall delete-then-recreate contradicts step 5's explicit "do NOT unregister first" prohibition — no justification for asymmetric treatment | MEDIUM | Opus | Accepted — asymmetry note added explaining `New-NetFirewallRule` has no `-Force`; missing rule is recoverable (re-run script) unlike missing Task Scheduler task |
| R9-M3 | S-002 step 4 does not document `publish/` source path resolution — operator running from a different directory gets confusing `ItemNotFoundException`; no pre-flight guard if `publish/` absent | MEDIUM | Opus | Accepted — step 4 updated: path resolved as `$PSScriptRoot\..\publish\`; pre-flight guard added (abort with clear error if `PcsRemote.TrayHost.exe` not found) |
| R9-M4 | S-005 item 10 verifies only banner appearance/disappearance — a build where the banner works but `RefreshScoreboard`/`ChangeMatch` still execute would incorrectly pass deployment smoke testing | MEDIUM | GPT | Accepted — item 10 extended: while manual mode is active, click "Refresh Scoreboard" and verify warning notification + no action taken |
| R9-L1 | S-004 §§1, 8, 9 say "a few seconds" but S-005 item 3 pass threshold is 30s — ONLOGON startup on cold boot takes 10–20s; "a few seconds" creates false expectations | LOW | Opus | Accepted — all three instances updated to "within about 30 seconds" |
| R9-L2 | S-004 §5 says "pauses all automation" without noting that an in-flight trigger is allowed to complete before remote commands are rejected (H-SC-4 contract in IS-005 S-003) | LOW | GPT | Accepted (downgraded from MEDIUM) — §5 reworded: notes immediate blocking of new remote commands; adds sentence that an already-running action completes before manual mode takes effect |
| R9-I1 | S-003 "verbose logging" terminology imprecise — Serilog `Verbose` and `Debug` are distinct levels; `.MinimumLevel.Debug()` is the correct fix | INFO | Opus | Accepted — changed to "debug-level logging" throughout |
