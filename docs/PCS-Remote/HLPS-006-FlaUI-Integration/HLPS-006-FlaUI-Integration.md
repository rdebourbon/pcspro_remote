...
| I-U-7 | Behavior when cricket.exe is already running at `LaunchAndLoginAsync` call time — the policy must be chosen: (a) attach to existing process, (b) kill and relaunch, (c) transition to Error. | Agent | Yes — blocks process management implementation. Recommended policy: kill and relaunch (ensures clean state); must be confirmed before implementation begins. |
