...
| SA-U-2 | Does the "Start Live Stream" button show a confirmation dialog before starting? | User | **Yes — blocks FlaUI implementation (S-SA-2, S-SA-3)** | If a dialog appears, FlaUI automation must handle it. Mock and YouTube integration are unaffected. | ⚠️ Unresolved |
| SA-U-3 | Is "Stop" a toggle of the same button (text changes to "Stop Live Stream") or a separate button? | User | **Yes — blocks FlaUI implementation (S-SA-2, S-SA-3)** | Determines how FlaUI locates the stop control. Mock and YouTube integration are unaffected. | ⚠️ Unresolved |
