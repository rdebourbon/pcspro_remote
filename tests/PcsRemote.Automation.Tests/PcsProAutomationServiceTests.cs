...
public sealed class PcsProAutomationServiceTests
{
    ...
    /// to completion, with the process window immediately visible.
    /// Returns the service in <see cref="PcsProState.LoginScreen"/> state.
    /// </summary>
    private static async Task<(PcsProAutomationService Service, FakeProcessHandle Handle)>
        CreateServiceAtLoginScreenAsync()
    {
    ...
    [TestMethod]
    public async Task LaunchAndLoginAsync_WhenWindowImmediatelyVisible_RaisesStateChangedInLoginScreen()
    {
        var handle = new FakeProcessHandle { MainWindowVisible = true };
        var pm = new FakeProcessManager { StartedHandle = handle };
        ...
        states.Should().ContainInOrder(PcsProState.Launching, PcsProState.LoginScreen);
        svc.CurrentState.Should().Be(PcsProState.LoginScreen);
    }
    ...
    {
        var (svc, handle) = await CreateServiceAtLoginScreenAsync();
    ...
    {
        var (svc, _) = await CreateServiceAtLoginScreenAsync();
        ...
        // Expected sequence: Timeout→Error, Retry→NotRunning (from LoginScreen path)
    ...
    public async Task StopAsync_WhenInErrorState_TransitionsToNotRunningViaRetry()
    {
        ...
        // Cause crash → service moves to Error
    ...
    // -----------------------------------------------------------------------
    // AC-15b — StopAsync from LoginScreen: Timeout→Error→Retry→NotRunning
    // -----------------------------------------------------------------------

    [TestMethod]
    public async Task StopAsync_WhenInLoginScreen_TransitionsToNotRunningViaTimeoutRetry()
    {
        var (svc, _) = await CreateServiceAtLoginScreenAsync();
        svc.CurrentState.Should().Be(PcsProState.LoginScreen);
